...
if echo "$TOOLS_RESPONSE" | node -e "
  ...
  if (!Array.isArray(tools) || tools.length !== 11) process.exit(1);
  const names = tools.map(t => t.name).sort();
  const expected = ['get-component','get-framework-mapping','get-generation-context','get-layout','get-pattern','get-theme','get-token','list-antipatterns','list-components','search-tokens','validate-ui'];
  if (JSON.stringify(names) !== JSON.stringify(expected)) process.exit(1);
" 2>/dev/null; then
  echo "  OK: tools/list response valid (11 tools)"
else
